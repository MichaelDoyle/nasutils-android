 ...
 * NAS services details, and receive and process the response.</p>
 * 
 * <p>The host Activity must implement {@link OnRetrieveServices} 
 * or a ClassCastException will be thrown.</p>
 * 
 * <p>After the response is received and processed, the 
 * {@link OnRetrieveServices#onSuccess(Nas)} callback will be
 * made to the host activity.</p>
 * 
 * @author michaeldoyle
 *
 */
public class ReadyNasServicesReceiverFragment extends ReadyNasReceiverFragment {
    
    public static final String SAVED_INSTANCE_STATE_RESULT = "result";

    @SuppressWarnings("unused")
    private static final String TAG = ReadyNasServicesReceiverFragment.class.getSimpleName();
    
    private OnRetrieveServices mCallBack;
    ...
    public void setDetails() {
        ...
        if(activity != null) {
            if (mResult == null) {
                ...
                intent.putExtra(EXTRA_ACTION, ReadyNasServiceName.GET_SERVICES.getCode());
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mCallBack = (OnRetrieveServices) activity;
        } catch (ClassCastException e) {
            throw new ClassCastException(activity.toString()
                    + " must implement OnRetrieveServices");
        }
    }
     ...
     */
    public interface OnRetrieveServices {
        /**
