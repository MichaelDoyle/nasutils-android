 ...
 * NAS addon details, and receive and process the response.</p>
 * 
 * <p>The host Activity must implement {@link OnRetrieveAddOns}
 * or a ClassCastException will be thrown.</p>
 *
 * <p>After the response is received and processed, the
 * {@link OnRetrieveAddOns#onSuccess(com.md.nasutils.model.Nas)} callback will be
 * made to the host activity.</p>
 *
 * @author michaeldoyle
 *
 */
public class ReadyNasAddOnsReceiverFragment extends ReadyNasReceiverFragment {
    
    public static final String SAVED_INSTANCE_STATE_RESULT = "result";

    @SuppressWarnings("unused")
    private static final String TAG = ReadyNasAddOnsReceiverFragment.class.getSimpleName();
    
    private OnRetrieveAddOns mCallBack;
    ...
    public void setDetails() {
        ...
        if(activity != null) {
            if (mResult == null) {
                ...
                intent.putExtra(EXTRA_ACTION, ReadyNasServiceName.GET_ADDONS.getCode());
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mCallBack = (OnRetrieveAddOns) activity;
        } catch (ClassCastException e) {
            throw new ClassCastException(activity.toString()
                    + " must implement OnRetrieveAddOns");
        }
    }
     ...
     */
    public interface OnRetrieveAddOns {
        /**
