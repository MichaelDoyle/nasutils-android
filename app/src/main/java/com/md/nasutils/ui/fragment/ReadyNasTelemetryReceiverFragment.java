 ...
 * NAS Telemetry details, and receive and process the response.</p>
 * 
 * <p>The host Activity must implement {@link OnRetrieveTelemetry} 
 * or a ClassCastException will be thrown.</p>
 * 
 * <p>After the response is received and processed, the 
 * {@link OnRetrieveTelemetry#onSuccess(Telemetry)} callback will be
 * made to the host activity.</p>
 * 
 * @author michaeldoyle
 *
 */
public class ReadyNasTelemetryReceiverFragment extends ReadyNasReceiverFragment {
    
    public static final String SAVED_INSTANCE_STATE_RESULT = "result";

    @SuppressWarnings("unused")
    private static final String TAG = ReadyNasTelemetryReceiverFragment.class.getSimpleName();
    
    private OnRetrieveTelemetry mCallBack;
    ...
    public void refreshDetails(long since) {
        mResult = null;
        setDetails(since);
    }
    
    public void setDetails(long since) {
        ...
        if(activity != null) {
            if (mResult == null) {
                ...
                intent.putExtra(EXTRA_ACTION, ReadyNasServiceName.GET_TELEMETRY.getCode());
            ...
            } else if (mResult instanceof Telemetry) {
                mCallBack.onSuccess((Telemetry) mResult);
    ...
    public void onResult(Object result) {
        ...
        if (result instanceof Parcelable) {
            ...
            } else if (result instanceof Telemetry) {
                setDetails(0);
            }
        }
    }
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mCallBack = (OnRetrieveTelemetry) activity;
        } catch (ClassCastException e) {
            throw new ClassCastException(activity.toString()
                    + " must implement OnRetrieveTelemetry");
        }
    }
     ...
     */
    public interface OnRetrieveTelemetry {
        /**
         * Callback to the Activity when services is retrieved.
         */
        void onSuccess(Telemetry telemetry);
